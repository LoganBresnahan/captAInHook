...
- **`/deploy`** — ships the current build (ALL FOUR artifacts: native
  `captainShim` + single-file `captainHook` engine + committed `ui/` + the
  pinned `bosun` spawn wrapper, staged and swapped together) to the live hook
...
- env vars: `CAPTAINHOOK_LOG`, `CAPTAINHOOK_LOG_STDERR`, `CAPTAINHOOK_HARNESS_DIR`, `CAPTAINHOOK_DISPATCH_FILE`, `CAPTAINHOOK_HANDLERS_FILE`, `CAPTAINHOOK_WATCH_FILE`, `CAPTAINHOOK_PROBE`, `CAPTAINHOOK_COLDSTART`, `CAPTAINHOOK_IDLE_MS`, `CAPTAINHOOK_API_PORT`, `CAPTAINHOOK_SSE_HEARTBEAT_MS` (test knob)
