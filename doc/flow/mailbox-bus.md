...
```
   ~/.captainHook/watch.json          ← the SECOND consent document (d7)
     ...
     valid ──────────┘                          (malformed also warns)
        ...
        │   cursors ──▶ MailWatch.ReadMailboxes   one WatchedMailbox per cursor file, else the sessionless read
        │   nudges.jsonl ──▶ NudgeStore.Load ──▶ NudgeState   first-seen / quiet-since / nudged / the role's hour
        ▼
   WatcherBrain.Evaluate(WatchInput{mailboxes, presence, kinds, rules, state, NowMs})   ← PURE
        │
        │   per rule role:  unread? ─▶ channel exists? ─▶ admitted & past quietFor? ─▶ noLiveSession? ─▶ budgets?
   ...
   MailNudgeEvent.DispatchAsync
        ...
        │
        ▼  MailNudgeOutcome{Ran, dispatchId}
   NudgeStore.Record(state, nudge, outcome, now)      the CALLER's, after the fact
        ├─ Ran     ──▶ mail.nudge (the trail row) + charge perEnvelope + the role's hour
        └─ denied  ──▶ nothing on the mail lane, quiet clock restarted, budgets untouched
        │
        ▼
   NudgeStore.Save(state, now)   ──▶  nudges.jsonl   (ages, never stamps; append-only)
```
...
**The brain decides, and only decides (ADR-0017 d4, slice `watcher-brain`).**
`WatcherBrain.Evaluate` is a pure function: the store, the cursors, the
registrations, presence and the rules are read by its caller and handed over as
values with one monotonic number, `NowMs`; it hands back the nudges to raise
now, the state to keep, one deadline, and a line per rule-bearing role. Three
rules fix its protocol with the actor that will host it, so no later phase can
redefine the state's shape:

- **One deadline.** `NextCheckMs` is the earliest instant a re-evaluation could
  change the answer — the nearest quiet threshold, the nearest presence expiry,
  the nearest budget-window release, the projected re-arm of the nudges it just
  emitted — or null. The actor arms exactly that and replaces what it held: a
  second envelope while armed re-arms (the brain re-runs on its `mail.append`
  and the new minimum wins) and can never double-arm, because there is only ever
  one number to hold.
- **Disarm is not an operation.** A cursor advance that clears the condition
  makes the envelope leave every mailbox, so it leaves the state and the next
  evaluation arms nothing for it. If it reappears (a reaped cursor re-anchoring)
  its clock starts over.
- **`Record` is the caller's.** The verdict tracks every unread envelope but
  charges no budget and resets no quiet clock for the nudges it emits, because
  a nudge `dispatch.json` denies must not spend a budget the operator's rules
  refused to let it use, and only the dispatch outcome knows. An uncharged
  record still restarts the quiet clock, so a denial recurs once per period —
  visible on the trail, never a tight loop.

Two readings were decided in the conservative direction. **Unread** is strict:
an envelope is unread for a role when it is pending in *every* mailbox of the
role that accepts it — one reader taking delivery is the role having heard it,
and a dead cursor's held-forever mail (the reaper's shape) is not a reason to
spend tokens; a unicast is judged by its one mailbox; a role with no cursor at
all is read sessionless and everything ever sent to it is unread, which is the
truth for a role nobody has picked up. **Quiet** counts from the moment the
brain first saw the envelope unread — never from the envelope's `ts`, a
sender's wall clock — and restarts on `Record`, so a second poke waits the full
period. **Live** is a freshest dispatch within 10 minutes, the canvas's own
idle→stale boundary, through the one comparison `RolePresence` owns; a `--as`
mailbox never looks live. Per envelope the first rule whose priority admits it
governs threshold and budget, so an urgent-fast rule and an ambient-slow one
coexist for one role; `perRoleHour` is the strictest among the admitting rules.

**Across a restart, the state travels as durations.** A monotonic stamp means
nothing in another process, so `NudgeState.ToAges(now)` writes ages and
`FromAges(ages, now)` re-derives stamps — two subtractions on one clock each,
no `DateTime` anywhere. The consequence is stated rather than hidden: time the
daemon was not running is not counted. Six of ten minutes quiet at idle-exit is
six minutes quiet on the next start and due four minutes later; a deadline that
had already fallen is due at once (the ADR's N2 promise); budget windows stretch
across the gap. Fewer nudges, again.

**`captainHook mail watch --once` runs the same brain from the CLI and prints.**
It is verification, refused without `--once` so it can never read as the
schedule the ADR rejected, and it names what it cannot see instead of guessing:
presence is the daemon's own view, so the CLI claims only the calling session
on its stdin (hook-shaped JSON or an exec-wire envelope) at age 0 — behind a
hook that is the window that fired it, on a bare terminal it is nobody; the
brain's memory it READS (`nudges.jsonl`) and never writes, so its quiet clocks
are the daemon's own, and with no file yet every unread envelope is first seen
now; `--as-if-quiet` is how an operator sees what the rules would do once time
has passed. It writes no cursor and moves none, prints the per-role
standing, the nudges it WOULD raise with reason and digest, and the one
deadline, and leaves one `watch.verdict` line on the trail — so a window that
wires it behind a hook for a dogfood week leaves a record of what the brain
would have done. *Where* it prints depends on who asked: on a terminal
(hook-shaped JSON or nothing on stdin) the report is stdout; behind a hook —
an exec-wire envelope on stdin — stdout is the ANSWER channel the engine reads
one JSON line from and kills the child on anything else, so the report goes to
stderr, the trail line is written first, and stdout carries exactly `mail
digest`'s `{"effect":"noop","dispatchId"}`. (As first built it printed the
report on stdout in both shapes, which behind a hook would have failed every
dispatch with `exec.protocolError` and lost the `watch.verdict` line to the
kill — caught in the 2026-08-18 review pass.) `MailWatch.ReadMailboxes` is the one gatherer for the CLI and
the actor alike: one mailbox per cursor file, keyed by the file's instance; the
sessionless read for a role with no cursor; and one for every `role@instance`
the ledger addresses that has no cursor yet — a `--as` mailbox not yet fired, an
answer to a window whose cursor was reaped — because a unicast no cursor accepts
would otherwise be watched by nobody. Its broadcast history reads pending from
the anchor, and the intersection rule keeps that honest.

Two more things the skeptic pass forced into words. The digest a nudge carries
is the real renderer's and therefore **capped at whole items**, so a nudge names
and charges only the envelopes it carries: the tail stays due, un-nudged, and
the verdict arms `now` so the next evaluation carries it — as first written the
cap silently spent `perEnvelope` on mail no turn ever saw. And four limits are
kept rather than papered over, in `WatcherBrain.cs`'s header: presence reaches
a role only through its cursors (a window that has never been handed mail is
invisible to `noLiveSession`); a role whose last cursor is reaped is read
sessionless and its retained history reads unread; quiet accrues only while the
daemon runs, so a `quietFor` longer than the idle window is reachable only while
...
### The memory, and the record

The brain is pure, so everything it remembers between evaluations is a value it
is handed — and `NudgeStore` (slice `nudge-state-and-trail`) is the only place
that value touches a file, and the only place a `mail.nudge` line is written.

**What is stored is AGES, never stamps.** A monotonic number means nothing in
another process, so the state leaves as durations from the moment it was written
(`ToAges`) and comes back as stamps re-derived from the moment it was read
(`FromAges`), with the gap not counted — the restart story above, made durable.

**The file is append-only JSONL, and that is the concurrency story.** Each save
appends one line holding the whole state; a read takes the LAST line that
parses. There is no lock, because there is one writer by design (the daemon's
watcher; `--once` is dry). What a lock would buy is bought by the reader: a
crashed or interleaved append can only leave a line that does not parse, and
such a line is SKIPPED — losing the tail costs one save (a state whose ages
under-count elapsed quiet), and losing every line is a REANCHOR to
`NudgeState.Empty`, every clock restarted. Both directions are *fewer nudges,
later*, and both say so on the trail (`watch.stateTorn`, `watch.stateReanchor`).
Past `CompactAtBytes` a save rewrites the file to its single current line
through a rename, so it cannot grow forever. A line whose `v` this build does
not know — or one carrying a member it does not know — is skipped rather than
...
stated where it can drift); there is no `sessionId` (a nudge belongs to a role
and carries no window — `dispatchId` is what joins the row to the woken turn's
`dispatch.start → exec.spawn → exec.exit`); and `budget` is NUMBERS rather than
the sentence in `reason`, rendered from the one `MailNudgeBudget` the reason's
clause is also rendered from. A denied nudge writes no row at all: nobody was
woken, its record is `nudge.denied`, and the state takes it uncharged so the
refusal recurs once per quiet period rather than once per evaluation. The Mail
...
### The loop closes (slice `e2e-stub-runner-loop`)

Every box above is pinned on its own. This is the one place they are joined and
driven — a human asks a role nobody is sitting in, and the answer comes back at
the asker's next prompt, with no human in the middle and no model in the loop:

```
  mail send ──▶ trail row ────▶ watcher ──▶ MailNudge ──▶ turn-claude.sh
  (asker, CLI)  (mail.append)  (watch.json) (dispatcher)      │ pickup FIRST
        ▲                                                     ▼ (sessionless box)
        │                                              stub `claude` (the ONLY stub)
        │                                                     │
        └── mail digest ◀── mail.append ◀── mail send ◀────────┘
            (the asker's next UserPromptSubmit)   kind: answer, inReplyTo
```

Three things this shape proves that no unit reaches. **The daemon is poked by
nobody**: the only thing that reaches the watcher is the `mail.append` row a CLI
in another process wrote to the trail — which is exactly how it works live,
because neither `mail send` nor `mail digest` is ever raised in-daemon. **The
ledger joins end to end**: the turn's `mail.deliver` carries the *nudge's*
`dispatchId`, `hookEvent: UserPromptSubmit` (the seam the payload opened) and no
`sessionId`, so the poke, the pickup and the cursor it left are one story a
canvas can draw. And **the loop stops**: the turn's own pickup and answer are
trail rows the watcher re-evaluates on purpose, and one nudge is still the whole
bill for the exchange (N1) — the budget, not a filter, is what ends it.

The stub stands in for the woken model and nothing else; the payload, the verbs,
the dispatcher and the watcher are the shipped artifacts. `mail ask --wait` is
not in the picture — the asker closes the loop the way every live window does,
on its next digest — so that verb, when it lands, is a second way to read the
same answer rather than a second loop.
...
| THE LOOP, END TO END (ADR-0017 phase 6, slice `e2e-stub-runner-loop`) — one test drives append → rule → `MailNudge` → the shipped turn payload → its sessionless pickup → an answer on the bus → the asker's next digest, with a stub `claude` as the ONLY stand-in and zero tokens spent. The daemon is poked by nothing: a CLI in another process writes `mail.append` to the trail the watcher tails. Asserts the ledger's join (the turn's `mail.deliver` carries the NUDGE's `dispatchId`, `hookEvent: UserPromptSubmit`, no `sessionId`), the sessionless cursor (`cursor.reviewer..json`), `inReplyTo` surviving the whole chain, exactly-once delivery to the asker, and that the turn's OWN rows re-trigger an evaluation which raises no second nudge (N1). The ADR row's "stub payload fires `captainShim hook user-prompt-submit`" is superseded by phase 4: guard 1 is kept verbatim, so a woken turn fires no hooks at all | `dotnet/captainHookTests/WatcherLoopE2ETests.cs` (1) over the real `DaemonHost`, the committed `examples/payloads/turn-claude.sh`, and `captainHook mail send` / `mail digest` as child processes; sandbox `HOME` + throwaway rendezvous + `FakeClock`, so the live tree is never touched and no idle window is ever waited out |
