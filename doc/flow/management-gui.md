...
| the Mail canvas — the bus, watched LIVE (ADR-0016 d14; the mechanism itself is `doc/flow/mailbox-bus.md`) | `web/src/MailPanel.tsx` (`MailPanel`, `MailStreamBadge`, `Spine`, `Lane`, `Glyph`, `Track`, `LaneHeads`, `Detail`; `data-island="mail"`, `data-lane`, `data-glyph`, `data-track`, `data-mark`, `data-envelope-card`, `data-tier`, `data-mail-stream`; the choreography's own hooks `data-arrival` and `data-motion`) — reads `GET /api/v1/mail` ONCE and has no other verb. Since slice 6a that one read also carries the trail's `mail.deliver` history (`deliveries` + `deliveriesComplete`), so a pickup that predates the page reads ✓ rather than *before cursor · no record*, and the detail card's `[data-no-record]` says which of the two an absent record means |
| the Mail stream — snapshot joined to the live fold (slice 5) | `web/src/mailStream.ts` (`runMailStream`, `startMailStream`, `MailStreamPorts`): fetch the snapshot, seed, then open a SECOND subscription at the stamp the snapshot carries (`MailDto.trailEventId`) — zero loss, zero duplicate. A second subscription rather than a filter over the trace's, because the resume id is OPAQUE (ADR-0009 d2) so a watermark comparison is not available, and the trace's buffer is `TRACE_CAP`-capped, which is right for a log and silent corruption for a reduced picture. `resyncing` = the reducer raised `resnapshot`; the driver re-seeds and re-anchors at the NEW stamp |
| the four motions | `web/src/styles.css` (`mail-arrive`, `mail-jump`, the `.track-cursor` transform transition and its `[data-motion="reanchor"]` suppression, the spend/presence transitions, all under `prefers-reduced-motion`) keyed on `MailGlyph.arrival` / `MailTrack.motion` from `web/src/mailCanvas.ts` — the canvas computes no motion of its own |
...
| the reduced bus in the store | `web/src/store.ts` (`mail`, `mailUi`, `mailStream`, `seedMailSnapshot`, `foldMailFrame`, `setMailStream`, `setMailView`, `setMailSelected`) over `web/src/mail.ts` — an interpolator between snapshots (N8), never a second store |
