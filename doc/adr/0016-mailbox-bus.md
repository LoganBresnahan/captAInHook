...
| decision | lives in |
|---|---|
| d2 — envelope + strict parser | `MailEnvelope` (`TryParse`/`TryParseLine`, `MailSender`, `MailKind`, `MailPriority`) in `dotnet/captainHook/Mail/MailEnvelope.cs`; `MailEnvelopeTests.cs` (115 — 98 parse cases + `MailAddressTests` 17, ADR-0018 d2's grammar). `ts` is stamped by the verb, format-unvalidated by design |
