...
provenance (d8), and — slice 5 — the routing itself (d4). A mailbox can be
NAMED, a named one is durable, and a `role@instance` envelope now reaches it and
only it. Nothing yet WRITES a forward, nothing reaps, and the canvas does not
draw instances. Status flips at `docs-instance-addressing`. Build order via
`/adr-plan`, below.
