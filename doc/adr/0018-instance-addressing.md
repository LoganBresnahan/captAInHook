...
unicast TTL refusal (d5), instance registration (d3), and `forwardedFrom`
provenance (d8). A mailbox can now be NAMED and a named one is durable;
nothing yet ROUTES a `role@instance` envelope to it (`plan-unicast`), so
unicast mail still reaches no one, and nothing yet WRITES a forward. Status flips at `docs-instance-addressing`. Build order via
`/adr-plan`, below.
