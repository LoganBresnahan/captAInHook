...
| 4 | `reaper-payloads` — the deterministic fallback (`forward-all-to-role` / `drop-all`) and the LLM reaper (`claude -p` over the dead box's pending, forward/drop/hold, then `mail reap`); **the authority decision is made and recorded HERE** | opus | medium | Stub-payload tests pin the verbs it runs; the LLM reaper is dogfood-only until authority is decided (N3). **It has a live subject as of 2026-08-19** (`doc/dogfood/2026-08-19-the-robot-channel.md`): the first robot turn created the role's sessionless `cursor.reviewer..json` beside `cursor.reviewer.84abfc7c-…json`, an unregistered instance mailbox whose window closed days ago and whose session is not live — d6's dead mailbox exactly, on the maintainer's own tree. Until this slice lands a `reaper` rule can only log `unserved`, so the detection half built in `watcher-dead-mailbox-rule` currently has no hands. |
