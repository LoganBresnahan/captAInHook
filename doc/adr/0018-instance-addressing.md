...
everything. Slices 1–3 of 13 have since landed — the address grammar (d2), the
unicast TTL refusal (d5), and instance registration (d3). A mailbox can now be
NAMED and a named one is durable; nothing yet ROUTES a `role@instance`
envelope to it (`plan-unicast`), so unicast mail still reaches no one. Status flips at `docs-instance-addressing`. Build order via
`/adr-plan`, below.
