...
everything. Slices 1–2 of 13 have since landed — the address grammar (d2) and
the unicast TTL refusal (d5), both parser-level and both permanent-by-nature;
nothing yet ROUTES on an instance, so the bus still behaves exactly as
ADR-0016 left it. Status flips at `docs-instance-addressing`. Build order via
`/adr-plan`, below.
