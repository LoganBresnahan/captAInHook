...
provenance (d8), slice 5 the routing itself (d4), and slice 6
...
key IS the address. A `role@instance` envelope reaches its one mailbox; a
request can carry `replyTo` and an answer goes there. Nothing yet WRITES a
forward, nothing reaps, and the canvas does not draw instances. Status flips at
