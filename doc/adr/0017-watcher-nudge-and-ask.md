...
| 4 | `turn-claude-payload` — `examples/payloads/turn-claude.sh` (exec-wire envelope on stdin ⇒ `claude -p "<digest>"` in the role's workspace, fresh session) + handlers.json entry + a stub-claude test pinning argv | watcher | opus | medium | Deterministic argv pin. **Resolve the reentrancy question here:** the driven claude's own `UserPromptSubmit` MUST fire (d6 wants a real cursor), so ADR-0010 N7's `--setting-sources ""` guard cannot be copied verbatim — the structural bound is the nudge budget + `dispatch.json`. **And resolve the corpse question here** (brain review, 2026-08-18): a fresh session per turn leaves an ephemeral `role@<sessionId>` cursor per turn, and ADR-0018 d6's dead-mailbox rule cannot tell it from a human window that died — with a `reaper` rule on, every past robot turn becomes a reaper wake once the next broadcast lands. The brain has no fact to exempt it on; the payload must read `--as` a REGISTERED durable mailbox (`RoleKinds.IsRegisteredMailbox` exempts it), scoped so the human's window in the same cwd does not inherit the name (a handlers.json `--as` is per-registration, not per-session — `dispatch.json` `project`/harness scoping, or a payload-side env the registration reads, are the candidates; decide and pin with a test that a driven turn's cursor is never a `WatchVerdict.Dead` candidate). |
