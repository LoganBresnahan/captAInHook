...
| macOS ships NO `setsid(1)` | The utility is util-linux; stock macOS lacks it (installable via brew's util-linux). The syscall exists — only the wrapper binary is missing, and .NET exposes no setpgid/setsid seam around fork/exec (the reason the wrapper is used at all). | ⚠ On stock macOS every exec-handler spawn runs `pgroup=false`: tree-walk kills only, the reparented-grandchild hole open. Loud per spawn by design; the degrade IS the macOS baseline until either brew-setsid is documented as a recommendation or a native spawn helper earns an ADR. (ADR-0012 N2's "macOS unproven" includes this.) |
